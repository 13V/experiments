#!/usr/bin/env python3
"""
Deploy manna/site/ to Vercel as a static project. No dependencies beyond Python 3.

  VERCEL_TOKEN=... [VERCEL_TEAM_ID=team_...] [VERCEL_PROJECT=manna] python3 manna/deploy-site.py [--preview]

Copies manna/config/*.json into manna/site/config/ (so a deploy always ships whatever the
...
every file under manna/site/ inline through the deployments API, waits for the deployment to
...
Adapted from locate/deploy-site.py: same script, different deployment root and project name.
"""
import base64
import json
import os
...
PROJECT = os.environ.get("VERCEL_PROJECT", "manna")
HERE = os.path.dirname(os.path.abspath(__file__))  # .../manna
ROOT = os.path.join(HERE, 'site')                  # .../manna/site, the deployment root
CONFIG_SRC = os.path.join(HERE, 'config')          # .../manna/config, the source of truth
CONFIG_DST = os.path.join(ROOT, 'config')          # .../manna/site/config, what ships
...
if not os.path.isdir(CONFIG_SRC):
    sys.exit(f'missing {CONFIG_SRC} — run from a checkout that has manna/config/*.json')
...
body = {
    ...
    'meta': {'source': 'manna/deploy-site.py'},
}
if not PREVIEW:
