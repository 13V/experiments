...
Paste everything below this line into a fresh Claude Code session. Fill in the two placeholders first.

---

You are moving the Stonk Packs project out of the shared experiments repo into its own repository. Do it in one pass, verify, push, and report. Ask nothing until the end unless a placeholder below is missing.
...
## Destination
- `<NEW_REPO_URL>` (for example `https://github.com/<owner>/stonk-packs`). If it does not exist, create it (private unless told otherwise) with default branch `main`. If it already has files, work on a branch named `import-stonk-packs` and open no pull request until asked.
...
1. `README.md` now starts at `# Stonk Packs`. In its layout block, keep the paths, add one line each for `scripts/keccak.js` and `scripts/secp256k1.js`, and replace every `13V/experiments` link with the new repo. Every technical fact stays exactly as written (chain id 4663, USDG address, the `block.number` notes, the test count).
2. `site/config.js`: set `github` to the new repo URL. Leave `contract`, `deployBlock` and `chainRoot` empty unless values are provided. Leave `social.*` empty unless provided.
